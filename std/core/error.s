...
def WHICHERR()
    ...
    doc "case of failure. Since it has no arguments, it can be provided"
    doc "to main as an automatically created parameter"
    doc "that prints encountered error messages rather than silently"
    doc "exiting on unhandled errors. You can show errors by"
    doc "compiling with `--debug`. But WHICHERR has no overhead"
    doc "during runtime and can thus be used in production versions"
    doc "for minimal exit code checking support. Declaring it in the"
    doc "main's title is just declarative syntax sugar for calling."
    doc "Example:"
    doc "```python"
    doc "import std.core"
    doc "def main(CLI, new CHARS, WHICHERR)"
