...
function highlight(){
  var out = escapeHtml(editor.value).replace(
    /("(?:[^"\\]|\\.)*"|#.*|\b(?:if|while|for|in|else|try|type|fail|mut|edit|effect|unsafe_mut)\b|\b(?:rec|def|local|import|repo|as)\b|[+\-*/%=<>!&|^~:]+)/g,
    function(m){
      if(m[0]==='#') return '<span style="color:#777777">'+m+'</span>';
      if(m[0]==='"') return '<span style="color:#2c5b19">'+m+'</span>';
      if(/^(if|while|for|in|else|try|fail|mut|edit|unsafe_mut|effect|type)$/.test(m)) return '<span style="color:#7a4f7d">'+m+'</span>';
