...
def spmat(effect edit new FLOATS, nat rows, nat cols, nat nnz)
    doc "allocate a sparse matrix on a fresh buffer of sparse_elements"
    buf = sparse_element[].alloc nnz
    return spmat(buf.unsafe_ptr, rows, cols, nnz)

def spmat(sparse_element[] elements, nat rows, nat cols)  
    return spmat(elements.unsafe_ptr, rows, cols, len elements)

def get(spmat m, nat k)
    doc "get the k-th sparse element"
    if k>=m.nnz fail "index out of bounds"
    return m.unsafe_ptr+k*24

def mutget(edit spmat m, nat k)
    doc "mutable reference to the k-th sparse element"
    if k>=m.nnz fail "index out of bounds"
    return unsafe_mut m.unsafe_ptr+k*24

def mul(effect edit vec_allocator FLOATS, spmat m, vec v)
...
def mul(effect edit vec_allocator FLOATS, vec v, spmat m)
...
def mul(effect edit vec_allocator FLOATS, spmat m1, mat m2)
...
def todense(effect edit vec_allocator FLOATS, spmat m)
...
def print(effect console CLI, spmat m, cstr|blank endl)
...
def sum(effect edit vec_allocator FLOATS, spmat m, "row")
...
def sum(effect edit vec_allocator FLOATS, spmat m, "col")
