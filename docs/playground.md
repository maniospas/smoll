...
function toterminal(message){d.innerHTML+=ansiToHtml(String(message))+'\n';}
...
async function initPyodide(){
  try{
    ...
    pyodideReady = true;
    // ADD HERE — before any Python runs
    pyodide.globals.set('_js_cache_get', function(url) {
      return localStorage.getItem(CACHE_DL_PFX + url);
    });
    pyodide.globals.set('_js_cache_set', function(url, b64) {
      try { localStorage.setItem(CACHE_DL_PFX + url, b64); } catch (_) {}
    });
    toterminal('Pyodide loaded');
  } catch(err) { toterminal(err.message); }
}
...
async function runVM(){
  ...
  pyodide.globals.set('_js_print',function(msg){toterminal(String(msg));});
...
`);
  ...
  //toterminal(((performance.now()-t0)/1000).toFixed(3)+'s');
  setRunning(false);
}
...
btnClearCache.addEventListener('click', function() {
  ...
  toterminal('Cache cleared (' + removed + ' entries)');
});
