    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function highlightSmoll(raw){
          return escapeHtml(raw).replace(
            /("(?:[^"\\\\]|\\\\.)*"|#.*|\\b(?:if|while|for|in|else|return|try|fail|mut|edit|unsafe_mut)\\b|\\b(?:def|local|import|repo|as)\\b|[+\\-*/%=<>!&|^~:]+)/g,
            function(m){
              if(m[0]==='#') return '<span style="color:#777777">'+m+'</span>';
              if(m[0]==='"') return '<span style="color:#2c5b19">'+m+'</span>';
              if(/^(if|while|for|return|in|else|try|fail|mut|edit|unsafe_mut)$/.test(m)) return '<span style="color:#7a4f7d">'+m+'</span>';
